// Migration: fügt das neue Feld `status : Text` zum Kanzlei-Record hinzu.
//
// Hintergrund: die neue `deactivateKanzlei`-Aktion (Super-Admin only) setzt
// den Kanzlei-Status auf "inaktiv" (Daten bleiben erhalten — nur der Status
// ändert sich). Physisches Löschen erfolgt über `deleteKanzlei`. Bisher hatte
// der Kanzlei-Record kein Status-Feld; diese Migration ergänzt es mit dem
// Default-Wert "aktiv" für alle bestehenden Kanzleien, damit stabile Daten
// nicht kaputtgehen.
//
// OldActor entspricht der NewActor des Vorgänger-Files
// (20260724_200000.mo) — d.h. die aktuell deployed State-Shape OHNE
// Kanzlei.status. NewActor erweitert den Kanzlei-Record um status = "aktiv".
//
...
module {
  ...
  // OldKanzlei: State-Shape VOR dieser Migration (ohne status).
  type OldKanzlei = {
    id : KanzleiId;
    name : Text;
  ...
  // NewKanzlei: State-Shape NACH dieser Migration (mit status = "aktiv").
  type NewKanzlei = {
    id : KanzleiId;
    name : Text;
    defaultStundensatz : Nat;
  ...
  // Vorgänger-Files 20260724_200000.mo). Kanzleien verwenden OldKanzlei
  // (ohne status).
  type OldActor = {
  ...
  // NewActor: State-Shape NACH dieser Migration. Kanzleien verwenden NewKanzlei
  // (mit status = "aktiv" für bestehende Einträge).
  type NewActor = {
  ...
  // Migration: ergänzt bestehende Kanzleien um status = "aktiv".
  // Alle anderen Collections werden unverändert durchgereicht.
  public func migration(old : OldActor) : NewActor {
    let newKanzleien = old.kanzleien.map<KanzleiId, OldKanzlei, NewKanzlei>(
      func(_id, k) {
        {
          k with
          status = "aktiv";
